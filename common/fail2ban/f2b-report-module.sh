...
generate_f2b_report() {
    ...
    if [ "$format" = "json" ]; then
        # JSON output
        "$SCRIPT_DIR/f2b-geoban.sh" -a -j -t 20 -f json
    else
        # Text report
        ...
        "$SCRIPT_DIR/f2b-geoban.sh" -a -j -t 20
        
        echo ""
        echo "Geographic Distribution:"
        "$SCRIPT_DIR/f2b-geoban.sh" -a -t 999 | tail -n +4 | awk '{for(i=3;i<=NF;i++) printf "%s ", $i; print ""}' | \
